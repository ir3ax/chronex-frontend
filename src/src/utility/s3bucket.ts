...
export const uploadToS3 = async (images: File[]) => {
  const promises = images.map(async (image: File, index: number) => {
    ...
    try {
      ...
      const location = `https://${import.meta.env.VITE_S3_BUCKET_NAME}.s3.${import.meta.env.VITE_S3_REGION}.amazonaws.com/${params.Key}`;
